...
const StatsSkeleton = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
    {Array.from({ length: 4 }).map((_, i) => (
      <div key={i} className="p-5 rounded-2xl shadow-md bg-white space-y-4">
...
function Dashboard() {
  ...
  const endpoints = {
    ...
    inactiveUsers:
      "https://api.emibocquillon.fr/admin/dashBoard/inactive-users",
    projectsCreatedToday:
      "https://api.emibocquillon.fr/admin/dashBoard/projects-created-today",
  };
    ...
    const fetchData = async () => {
      try {
        ...
        setStats({
          ...
          projectsCreatedToday:
            projectCreatedTodayRes.data.projectsCreatedToday ?? 0,
        });
      } catch (err) {
...
const activityBadge = (type) =>
  ...
  const planBadge = (plan) =>
    plan?.toLowerCase().includes("plus")
      ? "bg-emerald-100 w-26 text-emerald-600"
